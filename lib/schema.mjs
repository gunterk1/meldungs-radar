...
// RELEVANZ ist domänenfrei und bleibt hier. Das Bereichs-Enum und das
// Plausibilitätsfenster kommen aus dem Profil — sie sind das Einzige an dieser
// Prüfung, das vom Thema abhängt.
export { RELEVANZ } from './profil.mjs';
 ...
 */
export function klassifikationPruefen(roh, profil, heute = new Date()) {
  const BEREICHE = profil.bereiche;
  const fenster = profil.wirksamAb;
  try {
    ...
    if (wirksamAb !== null && wirksamAb !== undefined) {
      ...
      // Plausibilität: Das Fenster ist domänenabhängig. Eine Steuerregelung
      // darf zehn Jahre vorausdatiert sein, ein Sicherheitspatch nicht.
      // Wer ausserhalb des Fensters liegt, hat auch den Rest geraten.
      const jahre = (d.getTime() - heute.getTime()) / (365.25 * 864e5);
      if (jahre < fenster.minJahre || jahre > fenster.maxJahre) {
 ...
 */
export function jsonSchema(profil) {
  return {
    type: 'object',
    ...
    properties: {
      ...
      bereiche: { type: 'array', items: { type: 'string', enum: profil.bereiche } },
